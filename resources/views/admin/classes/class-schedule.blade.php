...
@section('content')
    ...
    `;
                        ...
                        // Chỉ cho click nếu ngày >= hôm nay
                        const today = new Date();
                        const eventDate = new Date(ev.date + 'T00:00'); // chuyển thành Date object
                        if (eventDate <= new Date(today.getFullYear(), today.getMonth(), today.getDate())) {
                            evEl.addEventListener("click", () => {
                                const selectedDate = ev.date;
                                window.location.href =
                                    `/qlgd/public/admin/classes/attendance/${CLASS_ID}?date=${selectedDate}`;
                            });
                        } else {
                            // Disable sự kiện cũ
                            evEl.style.opacity = 0.6;
                            evEl.style.cursor = "not-allowed";
                        }
